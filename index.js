...
<footer>
  © ${new Date().getFullYear()} Jontez TikTok Hub
</footer>
...
app.get("/", (req, res) => {
  ...
  res.send(page(
    ...
    <div class="hero">
      ...
      <a class="button" href="/auth/tiktok">
        Continue with TikTok
      </a>

      <div class="status">
        <strong>Service status:</strong> Online
      </div>
...
app.get("/privacy", (req, res) => {
  ...
  res.send(page(
    ...
    <ul>
      <li>To authenticate users through TikTok.</li>
      <li>To provide requested application functionality.</li>
      <li>To maintain and improve the application.</li>
      <li>To protect the application against unauthorized use.</li>
    </ul>
    ...
    <p>
      ...
      necessary to provide the requested service or comply with
      legal obligations.
    </p>
...
app.get("/health", (req, res) => {
  ...
  res.status(200).json({
    ...
    tiktok_client_key_configured: Boolean(CLIENT_KEY),
    tiktok_client_secret_configured: Boolean(CLIENT_SECRET)
  });
  ...
  if (!CLIENT_KEY || !CLIENT_SECRET) {
    ...
    return res.status(500).send(
      page(
        ...
        <p>
          TikTok credentials have not been configured on the
          server.
        </p>
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(400).send(
        page(
          ...
          <p>
          ${escapeHTML(error_description || "Authorization was not completed.")}
          </p>
    ...
    */
    ...
    if (!tokenResponse.ok || tokenData.error) {
      ...
      console.error("TikTok token response:", tokenData);
    ...
    /*
      Store tokens only in server-side/session storage in a
      production application.

      This demo does NOT display the access token to the user.
    */
